...
His other favorite word is “pueblo” but it is also worth mentioning that he has been using the words “avión “and “tren” a lot. why? :)

![image](https://user-images.githubusercontent.com/8701464/133873668-51aaec59-5434-4ba3-8007-ec08e1c0653e.png)

Although China is the most mentioned country, it has an almost uniform distribution in the last two years, but lately other countries have come to light

# Detecting nearest words to specific topics using Word Embeddings

FastText is an improved version of the Word2Vec model, It is working with n-grams, thus, it tries to predict the source context words given a target word or center words. It generates better word embeddings for out of vocabulary words because even if the words are very rares their n-grams are still shared with other surrounding words. **FasText is a good option for create Words Embeddings no dependent of the context**

![image](https://user-images.githubusercontent.com/8701464/133873677-5a495748-b194-4657-89a4-9e336bbccc5c.png)

The code above generated the result below:

- economía:[‘crecimiento’, ‘empleos’, ‘contrario’, ‘productivas’, ‘bajos’, ‘estabilidad’, ‘bienestar’, ‘crear’, ‘ingresos’, ‘peso’]
- educación:[‘reeducación’, ‘calidad’, ‘maestros’, ‘escolares’, ‘docentes’, ‘niveles’, ‘maestras’, ‘conocimientos’, ‘padres’, ‘deserción’]
- energética:[‘energético’, ‘industria’, ‘extranjeras’, ‘particulares’, ‘energéticas’, ‘pretendido’, ‘petróleo’, ‘extranjeros’, ‘energéticos’, ‘privatización’]
- migración:[‘unidos’, ‘cooperación’, ‘deportación’, ‘relación’, ‘centroamericanos’, ‘situación’, ‘relaciones’, ‘mexicanos’, ‘nacionalidades’, ‘centroamericana’]
- alimentación:[‘productos’, ‘precios’, ‘productores’, ‘acciones’, ‘amortización’, ‘consumos’, ‘fomento’, ‘importante’, ‘especial’, ‘fomentaremos’]
- salud:[‘médico’, ‘enfermedad’, ‘covid’, ‘hospital’, ‘especialización’, ‘médicas’, ‘médica’, ‘tratamientos’, ‘doctor’, ‘medicina’]
- corrupción:[‘políticos’, ‘acabar’, ‘impunidad’, ‘cortar’, ‘presupuesto’, ‘principal’, ‘escaleras’, ‘nación’, ‘honesto’, ‘funcionarios’]

Nearest words seems to be quite well, but we will visualize to better understand what we are talking about

## Visualizing Word Embeddings using t-SNE and PCA
The code below is used for further visualizations, It will help us visualize .gif animations of T-SNE with two components and different perplexities and PCA with three components. it will be easier to visually check the similarity between words.

![image](https://user-images.githubusercontent.com/8701464/133873693-10c9854d-a143-49d4-ae4b-032aaf6b0542.png)

The below image shows 10 nearest words to each topic and perplexity 12

![image](https://user-images.githubusercontent.com/8701464/133873701-034d18b5-e4bc-49ca-a188-606da47593b2.png)

The below image shows 50 nearest words to each topic with multiple perplexities

![image](https://user-images.githubusercontent.com/8701464/133873709-6110a555-cda8-414c-9881-1e8eec6dec67.png)


The below image shows 10 nearest words to each topic with PCA (Three components)

![image](https://user-images.githubusercontent.com/8701464/133873718-162f028a-f1b2-4589-9cd4-933f0342d4c5.png)

The below image shows 200 nearest words to each topic with PCA (Three components)

![image](https://user-images.githubusercontent.com/8701464/133873721-32f2ec5f-7518-42d1-a8a5-995f402eb112.png)

# Sentiment Analysis
The code below will work for calculate sentiments scores for each speech and visualize them by presidential term, values close to 0 are negative and values close to 1 are positive, there is a threshold that I choosed for neutral speeches, for values greater than 0.4 and less than 0.6

![image](https://user-images.githubusercontent.com/8701464/133873727-1a76de61-4944-4164-9243-08bb912f5d3d.png)

The image below shows that during the presidential term of Peña nieto there was more density of amlo’s speeches with negative sentiments, however, since the beginning of his presidential term there is almost an uniformity between both sentiments.

![image](https://user-images.githubusercontent.com/8701464/133873730-15254ab0-4a6a-49be-a293-b7c5be967eaa.png)
